    ...
    <div class="content">
      <div class="search">
        <el-form :model="queryParams" ref="queryRef" :inline="true" label-width="68px">
          <el-form-item label="告警日期" prop="type">
            <el-date-picker v-model="queryParams.warnDate" value-format="YYYY-MM-DD" type="daterange" range-separator="~" start-placeholder="开始日期" end-placeholder="结束日期" />
          </el-form-item>
          ...
          <!-- <el-form-item label="消警类型" prop="cancelType">
            <el-select v-model="queryParams.cancelType">
              <el-option label="手动消警" :value="0"></el-option>
              <el-option label="自动消警" :value="1"></el-option>
            </el-select>
          </el-form-item> -->
          <el-form-item class="button-container">
            <el-button type="primary" icon="Search" :loading="searchLoading" v-hasPermi="['fire:alarmLog:list']" @click="getData">查询</el-button>
      ...
      <div class="table" ref="pageTableWrapper" style="height: 100%">
        ...
        <el-table :max-height="pageTableHeight" :data="tableData" border style="width: 100%; margin-top: 20px;" :header-cell-style="{ 'text-align': 'center' }" v-loading="tableLoading" element-loading-text="加载中...">
          ...
          <el-table-column label="操作" width="200" align="center">
            <template #default="scope">
              <el-button type="text" icon="View" v-hasPermi="['fire:alarmLog:query']" @click="detail(scope.row)">查看</el-button>
            </template>
          </el-table-column>
        </el-table>
...
let queryParams = reactive({
  ...
  pageNum: 1,
  total: 0,
  warnDate: [moment().format('YYYY-MM-DD'), moment().format('YYYY-MM-DD')]
})
...
function getData () {
  ...
  let params = {
    ...
    startTime: queryParams.warnDate[0],
    endTime: queryParams.warnDate[1],
    alarmStatus: 1
  }
...
function resetQuery () {
  ...
  // queryParams.cancelType = null;
  queryParams.smsPush = null;
  queryParams.warnDate = [moment().format('YYYY-MM-DD'), moment().format('YYYY-MM-DD')]
  getData()
}
...
function detail (row) {
  warnService.warnDetail(row.id).then(res => {
    ...
    form.cancelTime = res.data.cancelTime;
    dialogDetail.data.forEach((item, index) => {
      dialogDetail.data[index].value = res.data[item.prop] != null ? res.data[item.prop] : '暂无'
    })
    visible.value = true;
  })
}
