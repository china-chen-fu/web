...
<!--        <el-form-item label="系统模块">
         <el-select no-data-text="暂无数据" v-model="platformId" @change="onSelectPlatform">
           <el-option :key="index"
                      :value="item.platformId"
                      :label="item.platformName"
                      v-for="(item, index) in usePermission.platforms" />
         </el-select>
        </el-form-item>-->
        <el-form-item label="菜单权限">
          ...
          <el-tree class="tree-border" :data="menuOptions" show-checkbox ref="menuRef" node-key="id" :check-strictly="!form.menuCheckStrictly" empty-text="加载中，请稍候" :props="{ label: 'label',  children: 'children' }">
            <template #default="{ node, data }">
              <span class="custom-tree-node">
                <span>{{ node.label }} {{ getPlatformName(data.platformId) }}</span>
              </span>
            </template>
          </el-tree>
        </el-form-item>
...
const title = ref("");
const dateRange = ref([]);
const menuOptions = ref([]); // 需要显示的菜单
// const menuAllOptions = ref([]); // add by wangxin 20221207 缓存所有菜单
...
// 设置默认id add by wangxin 20221207
// const platformId = ref(usePermission.platforms[0].platformId);
...
function getMenuTreeselect () {
  menuTreeselect().then(response => {
    ...
    // menuAllOptions.value = response.data;
    // switchMenus();
  });
}
/** 所有部门节点数据 */
...
function getRoleMenuTreeselect (roleId) {
  return roleMenuTreeselect(roleId).then(response => {
    ...
    // menuAllOptions.value = response.data.menus;
    // switchMenus()
    return response;
  });
}
...
//   console.log(menuAllOptions.value)
//   menuOptions.value = menuAllOptions.value.filter(f => f.platformId == platformId.value)
// }
/// {{{{ add by wangxin 20230109 菜单树显示平台名称
function getPlatformName(platformId) {
  let pfs = usePermission.platforms.filter(f=>f.platformId == platformId)
  if (pfs != null && pfs.length > 0) {
    return " - " + pfs[0].platformName
  } else {
    return "平台未使用"
  }
}
