  ...
  <div v-if="timeList.length !== 0">
        ...
        <div class="highEnergy">
          ...
          <div class="highEnergy_right">
            <div class="highEnergy_data">
              ...
              <span class="highEnergy_data_time"
                >{{ timeList[0].startTime }}-{{ timeList[0].endTime }}</span
              >
            </div>
            <div class="highEnergy_use">
              最大值{{ timeList[0].deviceUsage }}kW·h
            </div>
          </div>
        </div>
      </template>
        ...
        <div class="lowEnergy">
          ...
          <div class="lowEnergy_right">
            <div class="lowEnergy_data">
              ...
              <span class="lowEnergy_data_time"
                >{{ timeList[0].startTime }}-{{ timeList[0].endTime }}</span
              >
            </div>
            <div class="lowEnergy_use">
              最小值{{ timeList[0].deviceUsage }}kW·h
            </div>
          </div>
        </div>
      </template>
    ...
    <div v-if="timeList.length > 1">
      <div class="energyList">
        <div
          class="energyList_item"
          v-for="(item, index) in timeList"
          :class="{ energyList_low: item.status === '2' }"
          :key="index"
        >
          <el-popover :width="256" placement="top-start" :teleported="false">
            <template #reference>
              <div class="energyList_item_icon">
                {{ item.status === '1' ? '高' : '低' }}
              </div>
            </template>
            <template #default>
              <div
                class="highEnergy"
                v-if="item.status === '1'"
                @click="showEchartHighLight(item)"
              >
                <div class="highEnergy_left">
                  <div class="highEnergy_icon">
                    <svg-icon
...
// import { showEchartHighLight } from '../hooks/DrawBarChart'
</script>
